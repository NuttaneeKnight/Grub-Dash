...
// middleware for checking if delivery location exists
...
// middleware for checking if there is a phone number 
function bodyHasMobileNumber(req, res, next) {
  const { data: { mobileNumber } = {} } = req.body
  // if there is a phone number , move onto next()
  if (mobileNumber) {
...
function bodyHasStatus(req, res, next) {
  ...
  // if the dish has a delivery status, move onto next()
  if (status) {
...
// middleware for checking if there is a dish in the order
function bodyHasDishesProp(req, res, next) {
  const { data: { dishes } = {} } = req.body
  // if the order contain 1 or more dishes, move onto the next()
  if (dishes) {
